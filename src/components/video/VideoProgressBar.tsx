...
export const VideoProgressBar: React.FC<VideoProgressBarProps> = ({
  ...
  const [progressPosition, setProgressPosition] = React.useState({ x: 0, width: 0 });

  // 更新进度条动画
  const updateProgressBar = React.useCallback((time: number) => {
    if (duration > 0) {
  ...
  // 处理进度条布局
  const handleProgressLayout = React.useCallback((event: any) => {
  ...
  // 计算触摸位置对应的时间
  const calculateTimeFromPosition = React.useCallback((touchX: number): number => {
