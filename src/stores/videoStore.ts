...
import { type VideoState, type VideoFilter } from '@/types/stateTypes';
  ...
  MiddlewareCombinations.videoStore(
    ...
    addVideo: (video: Video) => set((state) => {
      ...
      if (existingIndex >= 0) {
        ...
        newVideos[existingIndex] = { ...newVideos[existingIndex], ...video };
      } else {
        // 添加新视频
    ...
    updateVideo: (videoId: string, updates: Partial<Video>) => set((state) => {
    ...
    setVideos: (videos: Video[]) => set((state) => ({
      ...state,
      videos,
      pagination: {
        ...state.pagination,
    ...
    // 播放控制
    setCurrentVideo: (video: Video | null) => set((state) => ({
      ...state,
      currentVideo: video,
      playbackState: {
        ...state.playbackState,
    ...
    playVideo: (videoId: string) => set((state) => {
    ...
    addToFavorites: (videoId: string) => set((state) => {
      const newFavorites = new Set(state.favorites);
      newFavorites.add(videoId);
    ...
    removeFromFavorites: (videoId: string) => set((state) => {
      const newFavorites = new Set(state.favorites);
      newFavorites.delete(videoId);
    ...
    setFavorites: (videoIds: string[]) => set((state) => ({
      ...state,
      favorites: new Set(videoIds),
    })),
    
    // 观看历史
    addToWatchHistory: (history) => set((state) => ({
      ...state,
      watchHistory: [history, ...state.watchHistory].slice(0, 100), // 保留最近100条
    })),
    
    updateWatchProgress: (videoId: string, position: number, duration: number) => set((state) => {
      const historyEntry = {
        ...
        watchTime: Math.abs(position - (state.watchHistory.find(h => h.videoId === videoId)?.position || 0)),
      };
      
      const existingIndex = state.watchHistory.findIndex(h => h.videoId === videoId);
      let newHistory;
    ...
    setFilter: (filter: Partial<VideoFilter>) => set((state) => ({
      ...state,
    ...
    loadMoreVideos: () => set((state) => {
      ...
      return {
        ...
        pagination: {
          ...state.pagination,
          page: newPage,
          hasMore: newPage * state.pagination.pageSize < state.pagination.total,
        },
      };
    }),
...
export const videoSelectors = {
    ...
    return videos.filter(video => {
      ...
      const matchesFilter = Object.entries(currentFilter).every(([key, value]) => {
        ...
        switch (key) {
          ...
          case 'size':
            const { min: minSize, max: maxSize } = value as { min: number; max: number };
            return video.fileSize >= minSize && video.fileSize <= maxSize;
          case 'isFavorite':
            return state.favorites.has(video.id) === value;
          case 'isWatched':
            return state.watchHistory.some(h => h.videoId === video.id) === value;
          default:
            return true;
        }
      });
  ...
  // 观看历史
  getRecentHistory: (state: VideoState, limit: number = 10) =>
