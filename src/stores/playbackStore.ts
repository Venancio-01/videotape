...
export const usePlaybackStore = create<PlaybackStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    // 位置控制
    setPosition: (position: number) => set((state) => ({
      ...state,
      position: Math.max(0, Math.min(position, state.duration)),
    })),
    
    seekTo: (position: number) => set((state) => {
    ...
    seekForward: (seconds: number) => set((state) => {
      const newPosition = Math.min(state.position + seconds, state.duration);
      return {
        ...state,
    ...
    seekBackward: (seconds: number) => set((state) => {
    ...
    setLooping: (looping: boolean) => set((state) => ({
      ...state,
      isLooping: looping,
    })),
    
    // 缓冲状态
    setBuffering: (buffering: boolean) => set((state) => ({
      ...state,
      isBuffering: buffering,
    })),
    
    setBufferedPosition: (position: number) => set((state) => ({
      ...state,
    ...
    // 错误处理
    setError: (error: string | null, code?: string) => set((state) => ({
      ...state,
      error,
      errorCode: code || null,
      isPlaying: false,
    ...
    playAtIndex: (index: number) => set((state) => ({
      ...state,
      currentQueueIndex: Math.max(0, Math.min(index, state.queue.length - 1)),
      position: 0,
      isLoading: true,
    })),
    
    // 状态更新
    updatePlaybackState: (updates: Partial<PlaybackState>) => set((state) => ({
      ...state,
      ...updates,
    })),
    
    // 进度更新
    updateProgress: (position: number, duration?: number) => set((state) => ({
      ...state,
      position: Math.max(0, Math.min(position, state.duration)),
      duration: duration !== undefined ? Math.max(0, duration) : state.duration,
    })),
    
    // 缓冲更新
    updateBuffering: (isBuffering: boolean, bufferedPosition?: number) => set((state) => ({
      ...state,
      isBuffering,
      bufferedPosition: bufferedPosition !== undefined 
    ...
    // 部分重置
    resetPlaybackControls: () => set((state) => ({
      ...state,
      isPlaying: false,
      isPaused: false,
      isLoading: false,
...
export const playbackSelectors = {
  ...
  getCanPause: (state: PlaybackState) => 
    state.isPlaying && !state.isLoading,
  getCanSeek: (state: PlaybackState) => 
    !state.isLoading && state.duration > 0,
  getCanSkipNext: (state: PlaybackState) => 
    playbackSelectors.getHasNext(state),
  ...
  getIsVolumeMuted: (state: PlaybackState) => state.volume === 0 || state.isMuted,
...
// 创建记忆化 Hook
export const usePlaybackSelector = <T>(selector: (state: PlaybackState) => T): T => {
...
export const usePlaybackProgress = () => usePlaybackSelector(playbackSelectors.getProgress);
...
export const usePlaybackControls = () => usePlaybackSelector((state) => ({
...
export const usePlaybackSettings = () => usePlaybackSelector((state) => ({
  volume: state.volume,
