...
import { type UIState, type Notification } from '@/types/stateTypes';
...
export const useUIStore = create<UIStore>()(
  MiddlewareCombinations.uiStore(
    ...
    setSearchOpen: (open: boolean) => set((state) => ({
      ...state,
      isSearchOpen: open,
    })),
    ...
    setColorScheme: (colorScheme: 'light' | 'dark') => set((state) => ({
      ...state,
      colorScheme,
    })),
    
    updateColorScheme: () => set((state) => {
      if (state.theme === 'system') {
        const newColorScheme = typeof window !== 'undefined' && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
        return {
          ...state,
    ...
    setMultipleLoading: (loadingStates: Record<string, boolean>) => set((state) => ({
      ...state,
      loadingStates: {
        ...state.loadingStates,
        ...loadingStates,
    ...
    setMultipleErrors: (errors: Record<string, string | null>) => set((state) => ({
      ...state,
      errors: {
        ...state.errors,
        ...errors,
    ...
    updateNotification: (id: string, updates: Partial<Notification>) => set((state) => ({
      ...state,
    ...
    removeExpiredNotifications: () => set((state) => {
      const now = Date.now();
    ...
    // 便捷通知方法
    showSuccess: (title: string, message: string, duration = 3000) => set((state) => {
    ...
    showError: (title: string, message: string, duration = 5000) => set((state) => {
      const notification: Notification = {
    ...
    showWarning: (title: string, message: string, duration = 4000) => set((state) => {
      const notification: Notification = {
    ...
    showInfo: (title: string, message: string, duration = 3000) => set((state) => {
      const notification: Notification = {
    ...
    // 批量操作
    updateMultipleUIStates: (updates: Partial<UIState>) => set((state) => ({
      ...state,
      ...updates,
    })),
...
export const uiSelectors = {
  ...
  getError: (key: string) => (state: UIState) => 
    state.errors[key],
  getHasError: (key: string) => (state: UIState) => 
  ...
  getUnreadNotifications: (state: UIState) => 
    state.notifications, // 可以根据需要添加已读/未读逻辑
  getRecentNotifications: (state: UIState, limit: number = 5) =>
  ...
  getUIStateSummary: (state: UIState) => ({
    ...
    errorCount: Object.keys(state.errors).filter(key => state.errors[key] !== null).length,
  }),
...
export const useModalState = (modalType: string) => useUISelector(uiSelectors.getModalVisibility(modalType));
export const useNotifications = () => useUISelector(uiSelectors.getNotifications);
export const useLoadingState = (key: string) => useUISelector(uiSelectors.getIsLoading(key));
export const useErrorState = (key: string) => useUISelector(uiSelectors.getError(key));
export const useUIStateSummary = () => useUISelector(uiSelectors.getUIStateSummary);
