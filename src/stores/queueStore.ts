...
import { type Video, type Playlist } from '@/db/schema';
...
export const useQueueStore = create<QueueStore>()(
  MiddlewareCombinations.queueStore(
    ...
    // 队列管理
    setQueue: (queue: Video[]) => set((state) => ({
      ...state,
      queue,
      currentQueueIndex: queue.length > 0 ? 0 : -1,
    })),
    
    addToQueue: (video: Video) => set((state) => {
    ...
    addToQueueNext: (video: Video) => set((state) => {
    ...
    setCurrentQueueIndex: (index: number) => set((state) => ({
      ...state,
      currentQueueIndex: Math.max(-1, Math.min(index, state.queue.length - 1)),
    })),
    
    // 播放历史管理
    addToHistory: (video: Video) => set((state) => {
      const newHistory = [video, ...state.history.filter(v => v.id !== video.id)];
      return {
        ...state,
    ...
    removeFromHistory: (videoId: string) => set((state) => ({
      ...state,
    ...
    // 播放列表管理
    setPlaylists: (playlists: Playlist[]) => set((state) => ({
      ...state,
      playlists,
    })),
    ...
    updatePlaylist: (playlistId: string, updates: Partial<Playlist>) => set((state) => ({
      ...state,
    ...
    setCurrentPlaylist: (playlist: Playlist | null) => set((state) => ({
      ...state,
      currentPlaylist: playlist,
    })),
    
    // 从播放列表播放
    playPlaylist: (playlist: Playlist, startIndex: number = 0) => set((state) => ({
      ...state,
      currentPlaylist: playlist,
      queue: playlist.videos,
      currentQueueIndex: Math.max(0, Math.min(startIndex, playlist.videos.length - 1)),
    })),
    
    // 播放模式
    setRepeatMode: (mode: 'none' | 'single' | 'all') => set((state) => ({
      ...state,
      repeatMode: mode,
    })),
    ...
    toggleShuffle: () => set((state) => {
      ...
      if (newShuffleState) {
        // 打乱队列（保持当前播放的项目在第一位）
        const currentItem = state.currentQueueIndex >= 0 ? newQueue[state.currentQueueIndex] : null;
      ...
      return {
        ...
        queue: newQueue,
        currentQueueIndex: newShuffleOn && newQueue.length > 0 ? 0 : state.currentQueueIndex,
      };
    }),
    ...
    setShuffle: (shuffle: boolean) => set((state) => {
      ...
      return {
        ...
        isShuffleOn: shuffle,
        queue: newQueue,
        currentQueueIndex: shuffle && newQueue.length > 0 ? 0 : state.currentQueueIndex,
      };
    }),
    ...
    // 批量操作
    addMultipleToQueue: (videos: Video[]) => set((state) => {
    ...
    removeMultipleFromQueue: (indices: number[]) => set((state) => {
      const indexSet = new Set(indices);
      const newQueue = state.queue.filter((_, i) => !indexSet.has(i));
      
      let newIndex = state.currentQueueIndex;
      const removedBeforeCurrent = indices.filter(i => i < state.currentQueueIndex).length;
      
      if (indexSet.has(state.currentQueueIndex)) {
        newIndex = Math.min(newIndex - removedBeforeCurrent, newQueue.length - 1);
      } else {
    ...
    shuffleQueue: () => set((state) => {
      ...
      // 移除当前项目
      const filteredQueue = newQueue.filter((_, i) => i !== state.currentQueueIndex);
    ...
    // 播放列表到队列
    loadPlaylistToQueue: (playlist: Playlist) => set((state) => ({
      ...state,
      queue: playlist.videos,
    ...
    // 部分重置
    resetCurrentQueue: () => set((state) => ({
      ...state,
      currentQueueIndex: -1,
    })),
    
    resetPlaylists: () => set((state) => ({
      ...state,
      playlists: [],
      currentPlaylist: null,
    })),
...
export const queueSelectors = {
  ...
  getCanPlayPrevious: (state: QueueState) => queueSelectors.getHasPrevious(state),
...
export const usePlaybackNavigation = () => useQueueSelector((state) => ({
...
export const usePlaylistManagement = () => useQueueSelector((state) => ({
