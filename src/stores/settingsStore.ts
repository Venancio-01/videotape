...
export const useSettingsStore = create<SettingsStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    setFontSize: (fontSize: 'small' | 'medium' | 'large') => set((state) => ({
      ...state,
      fontSize,
    })),

    // 播放设置
    setDefaultPlaybackSpeed: (speed: number) => set((state) => ({
      ...state,
      defaultPlaybackSpeed: Math.max(0.25, Math.min(speed, 4.0)),
    })),

    setDefaultVolume: (volume: number) => set((state) => ({
      ...state,
      defaultVolume: Math.max(0, Math.min(volume, 1.0)),
    })),

    setDefaultQuality: (quality: 'auto' | 'low' | 'medium' | 'high') => set((state) => ({
      ...state,
      defaultQuality: quality,
    })),

    setAutoPlay: (autoPlay: boolean) => set((state) => ({
      ...state,
      autoPlay,
    })),

    setAutoNext: (autoNext: boolean) => set((state) => ({
      ...state,
      autoNext,
    })),

    // 缓存设置
    setMaxCacheSize: (size: number) => set((state) => ({
      ...state,
      maxCacheSize: Math.max(100, Math.min(size, 10240)), // 100MB - 10GB
    })),

    setCacheRetentionDays: (days: number) => set((state) => ({
      ...state,
      cacheRetentionDays: Math.max(1, Math.min(days, 365)),
    })),

    setAutoClearCache: (autoClear: boolean) => set((state) => ({
      ...state,
      autoClearCache,
    })),

    // 隐私设置
    setAnalyticsEnabled: (enabled: boolean) => set((state) => ({
      ...state,
      analyticsEnabled: enabled,
    })),

    setCrashReportingEnabled: (enabled: boolean) => set((state) => ({
      ...state,
    ...
    setLogLevel: (level: 'debug' | 'info' | 'warn' | 'error') => set((state) => ({
      ...state,
      logLevel: level,
    })),

    // 批量更新
    updateSettings: (updates: Partial<SettingsState>) => set((state) => {
      const newState = { ...state, ...updates };

      // 验证和约束数值
      if (newState.defaultPlaybackSpeed !== undefined) {
        newState.defaultPlaybackSpeed = Math.max(0.25, Math.min(newState.defaultPlaybackSpeed, 4.0));
      }

      if (newState.defaultVolume !== undefined) {
        newState.defaultVolume = Math.max(0, Math.min(newState.defaultVolume, 1.0));
      }

      if (newState.maxCacheSize !== undefined) {
        newState.maxCacheSize = Math.max(100, Math.min(newState.maxCacheSize, 10240));
      }

      if (newState.cacheRetentionDays !== undefined) {
        newState.cacheRetentionDays = Math.max(1, Math.min(newState.cacheRetentionDays, 365));
      }
    ...
    resetPlaybackSettings: () => set((state) => ({
      ...state,
    ...
    importSettings: (settingsJson: string) => {
      try {
        ...
        if (importedSettings.defaultPlaybackSpeed !== undefined) {
          updates.defaultPlaybackSpeed = importedSettings.defaultPlaybackSpeed;
        }
        ...
        if (importedSettings.crashReportingEnabled !== undefined) {
          updates.crashReportingEnabled = importedSettings.crashReportingEnabled;
        }
...
export const settingsSelectors = {
  ...
  getCrashReportingEnabled: (state: SettingsState) => state.crashReportingEnabled,
...
// 创建记忆化 Hook
export const useSettingsSelector = <T>(selector: (state: SettingsState) => T): T => {
...
export const useLanguage = () => useSettingsSelector(settingsSelectors.getLanguage);
export const useFontSize = () => useSettingsSelector(settingsSelectors.getFontSize);
export const usePlaybackSettings = () => useSettingsSelector(settingsSelectors.getPlaybackSettings);
export const useCacheSettings = () => useSettingsSelector(settingsSelectors.getCacheSettings);
export const usePrivacySettings = () => useSettingsSelector(settingsSelectors.getPrivacySettings);
export const useDeveloperSettings = () => useSettingsSelector(settingsSelectors.getDeveloperSettings);
export const useSettingsValidation = () => useSettingsSelector(settingsSelectors.getSettingsValidation);
export const useSettingsStats = () => useSettingsSelector(settingsSelectors.getSettingsStats);
...
export const usePlaybackSpeedOptions = () => settingsSelectors.getPlaybackSpeedOptions();
export const useQualityOptions = () => settingsSelectors.getQualityOptions();
export const useCacheSizeOptions = () => settingsSelectors.getCacheSizeOptions();
