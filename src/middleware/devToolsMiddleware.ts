  ...
  return (stateCreator: StateCreator<T, [], []>): StateCreator<T, [], []> => {
    ...
    return (set, get, store) => {
      ...
      if (performanceMonitor) {
        ...
        // 添加性能监控方法
        (result as any).getPerformanceStats = () => performanceMonitor.getStats();
        (result as any).resetPerformanceStats = () => performanceMonitor.reset();
      }
...
const createPerformanceMonitor = (config: Partial<PerformanceConfig> = {}) => {
  ...
  const getStats = () => {
    ...
    const slowUpdateRate = totalUpdates > 0 ? (slowUpdates / totalUpdates) * 100 : 0;
    
    // 计算最近 100 次更新的平均时间
    const recentUpdates = stateUpdates.slice(-100);
    const recentAvgTime = recentUpdates.length > 0 
      ? recentUpdates.reduce((sum, update) => sum + update.duration, 0) / recentUpdates.length
      : 0;
    ...
    return {
      ...
      minRecordedTime: Math.min(...stateUpdates.map(u => u.duration), Infinity),
