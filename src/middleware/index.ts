...
// 组合中间件
import { createPersistMiddleware, PersistConfigs } from './persistMiddleware';
import { createLoggerMiddleware, LoggerConfigs } from './loggerMiddleware';
import { createDevToolsMiddleware, DevToolsConfigs } from './devToolsMiddleware';
...
export const MiddlewareCombinations = {
  ...
  // 开发模式中间件（包含所有功能）
  development: <T>(storeName: keyof typeof PersistConfigs) => createCombinedMiddleware<T>(storeName, {
    persist: true,
    logger: true,
    devTools: true,
  ...
  // 生产模式中间件（仅持久化）
  production: <T>(storeName: keyof typeof PersistConfigs) => createCombinedMiddleware<T>(storeName, {
    persist: true,
    logger: false,
    devTools: false,
  }),
  ...
  // 测试模式中间件（无中间件）
  testing: <T>(storeName: keyof typeof PersistConfigs) => createCombinedMiddleware<T>(storeName, {
    persist: false,
    logger: false,
    devTools: false,
  }),
