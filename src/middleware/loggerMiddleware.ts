...
// 格式化时间戳
const formatTimestamp = (timestamp: number, format: LoggerConfig['timestampFormat']): string => {
...
class LoggerManager {
  ...
  private outputToConsole(log: LogEntry): void {
    ...
    // 输出到控制台
    (console as any)[method](...message, ...data);
  }
  ...
  return (stateCreator: StateCreator<T, [], []>): StateCreator<T, [], []> => {
    return (set, get, store) => {
      const wrappedSet: typeof set = (...args) => {
        ...
        // 检查是否应该记录此状态更新
        const shouldLog = !config.predicate || config.predicate(prevState, action);
      ...
      const storeWithLogger = {
        ...
        subscribeToLogs: (callback: (log: LogEntry) => void) => logger.subscribe(callback),
        updateLoggerConfig: (newConfig: Partial<LoggerConfig>) => logger.updateConfig(newConfig),
      };
      
      return storeWithLogger as any;
    };
  };
};
