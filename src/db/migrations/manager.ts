...
export async function runMigrations(): Promise<void> {
  try {
    ...
    // 获取已执行的迁移
    const executedMigrations = await db.select({ name: sql`name` }).from(sql`migrations`);
...
export async function rollbackMigration(migrationName: string): Promise<void> {
  try {
    ...
    // 查找迁移的回滚SQL
    const migrationFiles = await import('./migrations/migrations');
    const migration = migrationFiles.default.find(m => m.name === migrationName);
...
export async function getMigrationStatus(): Promise<{
  ...
  try {
    ...
    // 计算待执行的迁移
    const pendingNames = allNames.filter(name => !executedNames.includes(name));
...
export async function backupDatabase(backupPath: string): Promise<void> {
  try {
    ...
    // 复制所有表
    for (const table of tables) {
      await db.run(`CREATE TABLE backup_db.${table.name} AS SELECT * FROM ${table.name}`);
    }
